  ...
  methods: {
    ...
    changeActive(spuSaleAttrValue, arr) {
      arr.forEach((item) => {
        ...
        this.skuColor=spuSaleAttrValue.saleAttrValueName
    },
    // 修改产品个数
    changeSkuNum(event) {
    ...
    async addShopCar() {
      ...
      try {
        ...
        if(this.skuColor==''){
        this.$router.push({name:'addcartsuccess',query:{skuNum:this.skuNum,skuColor:this.spuSaleAttrList[0].spuSaleAttrValueList[0].saleAttrValueName}})
        }else{
        this.$router.push({name:'addcartsuccess',query:{skuNum:this.skuNum,skuColor:this.skuColor}})
        }
      } catch (error) {
        alert(error.message)
      }
